# Phase 11 — Oprava záměru: odrážky + test bin.ts

**Goal:** Opravit zdvojenou odrážku v renderProjectMd (non-goal začínající '- '/'* ' nezdvojí odrážku) a pokrýt interaktivní cestu bin.ts (readline + nabídka + zaručené close) testem, aby se po dotazu proces nezasekl.

## Steps
- [done] Oříznout vedoucí odrážku u non-goalů
- [done] Test 10-2: '- text' nedá '- - text'
- [done] Vyčlenit testovatelnou výplň z bin.ts
- [done] Test 10-1: close vždy + ask jen interaktivně
- [done] Adversariální self-review + finální kontrola
