...
describe("runAiAnalysis – orchestrátor (analyze/classify injektované, bez sítě)", () => {
  ...
  it("uříznutý výstup (stop_reason=max_tokens) → čistý skip s důvodem a STRUKTUROVANOU cenou, NE pád na JSON", async () => {
    // přesně reálný sonnet bug: thinking sežral max_tokens, JSON prázdný/uříznutý
    const usage: AiUsage = { inputTokens: 70000, outputTokens: 16000 };
    const analyze: AnalyzeFn = async () => ({ rawText: "", usage, stopReason: "max_tokens" });
    ...
    if (ai.kind === "skipped") {
      ...
      // Zub fáze 62: cena/usage nese skip i STRUKTUROVANĚ (číslem), ne jen v textu –
      // jinak ji orchestrátor krájeného běhu nesečte. Vyříznutí polí test shodí.
      expect(ai.usage).toEqual<AiUsage>(usage);
      expect(ai.costUsd).toBe(computeCostUsd(usage, "sonnet"));
      expect(ai.costUsd).toBeGreaterThan(0);
    }
  });

  it("prázdný výstup i s end_turn → čistý skip se strukturovanou cenou (ne pád na JSON.parse(''))", async () => {
    const usage: AiUsage = { inputTokens: 10, outputTokens: 5 };
    const analyze: AnalyzeFn = async () => ({ rawText: "   ", usage, stopReason: "end_turn" });
    const ai = await runAiAnalysis({ [AI_KEY_ENV]: "k" }, intentWith(["x"]), payload, "opus", analyze, classifyNone);
    expect(ai.kind).toBe("skipped");
    if (ai.kind === "skipped") {
      expect(ai.usage).toEqual<AiUsage>(usage);
      expect(ai.costUsd).toBe(computeCostUsd(usage, "opus"));
    }
  });

  it("beznákladový skip (žádné non-goaly) NEnese usage/costUsd (rozlišení 'nestálo nic' vs '$0')", async () => {
    const ai = await runAiAnalysis({ [AI_KEY_ENV]: "k" }, intentWith(null), payload, "opus", okAnalyze, classifyNone);
    expect(ai.kind).toBe("skipped");
    if (ai.kind === "skipped") {
      expect(ai.usage).toBeUndefined();
      expect(ai.costUsd).toBeUndefined();
    }
  });
...
describe("runAiCodeAnalysis – orchestrátor (analyze/classify injektované)", () => {
  ...
  it("žádné soubory → skipped, analyze se nezavolá", async () => {
    const analyze = vi.fn(okAnalyze);
    const ai = await runAiCodeAnalysis({ [AI_KEY_ENV]: "k" }, { text: "", includedFiles: [] }, "opus", analyze, classifyNone);
    expect(ai).toMatchObject({ kind: "skipped" });
    expect(analyze).not.toHaveBeenCalled();
  });
  ...
  it("uříznutý výstup (max_tokens) → čistý skip se STRUKTUROVANOU cenou, NE pád na JSON", async () => {
    const usage: AiUsage = { inputTokens: 70000, outputTokens: 16000 };
    const analyze: AnalyzeFn = async () => ({ rawText: "", usage, stopReason: "max_tokens" });
    ...
    if (ai.kind === "skipped") {
      ...
      // Zub fáze 62: strukturovaná cena pro orchestrátor.
      expect(ai.usage).toEqual<AiUsage>(usage);
      expect(ai.costUsd).toBe(computeCostUsd(usage, "sonnet"));
    }
  });

  it("beznákladový skip (síťová chyba) NEnese usage/costUsd", async () => {
    const analyze: AnalyzeFn = async () => {
      throw new Error("net");
    };
    const ai = await runAiCodeAnalysis({ [AI_KEY_ENV]: "k" }, payload, "opus", analyze, classifyNet);
...
describe("runAiLogicAnalysis – orchestrátor (brána na záměru, analyze/classify injektované)", () => {
  ...
  it("žádné soubory → skipped, analyze se nezavolá", async () => {
    const analyze = vi.fn(okAnalyze);
    const ai = await runAiLogicAnalysis({ [AI_KEY_ENV]: "k" }, intentWith(null, "Stavím CLI"), { text: "", includedFiles: [] }, "opus", analyze, classifyNone);
    expect(ai).toMatchObject({ kind: "skipped" });
    expect(analyze).not.toHaveBeenCalled();
  });
